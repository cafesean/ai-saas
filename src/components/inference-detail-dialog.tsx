...
interface InferenceDetailDialogProps {
  open: boolean
  onOpenChange: (open: boolean) => void
  inference: any
}

export function InferenceDetailDialog({ open, onOpenChange, inference }: InferenceDetailDialogProps) {
            ...
            <div className="rounded-md border p-4 max-h-[400px] overflow-auto">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-muted-foreground">{inference.id}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Timestamp</p>
                  <p className="text-sm text-muted-foreground">{inference.timestamp}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Model Version</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.modelVersion === 'object' ? JSON.stringify(inference.modelVersion) : inference.modelVersion || "Unknown"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Environment</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.environment === 'object' ? JSON.stringify(inference.environment) : inference.environment || "Production"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Client</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.client === 'object' ? JSON.stringify(inference.client) : inference.client || "API"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">IP Address</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.ipAddress === 'object' ? JSON.stringify(inference.ipAddress) : inference.ipAddress || "Unknown"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Request Headers</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.headers === 'object' ? JSON.stringify(inference.headers) : inference.headers || "None"}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-sm font-medium">Response Code</p>
                  <p className="text-sm text-muted-foreground">{typeof inference.responseCode === 'object' ? JSON.stringify(inference.responseCode) : inference.responseCode || "200"}</p>
                </div>
              </div>
            </div>
          </TabsContent>
