...
export const rateCardRouter = createTRPCRouter({
  ...
  getActiveRateCard: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rateCard = await db.query.rate_cards.findFirst({
        where: and(
          ...
          or(
            sql`${rate_cards.expire_date} IS NULL`,
            gte(rate_cards.expire_date, date)
          )
        ),
        with: {
          level_rates: {
  ...
  create: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      return await db.transaction(async (tx) => {
        // Create rate card
        const [newRateCard] = await tx.insert(rate_cards)
          .values({
            ...rateCardData,
            uuid: uuidv4(),
            created_at: new Date(),
        ...
        if (levelRates.length > 0) {
          ...
          await tx.insert(level_rates)
            .values(levelRates.map(rate => ({
              uuid: uuidv4(),
  ...
  update: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return await db.transaction(async (tx) => {
        // Update rate card
        const [updatedRateCard] = await tx.update(rate_cards)
          .set({
            ...rateCardData,
            updated_at: new Date(),
          })
        ...
        if (levelRates) {
          ...
          await tx.delete(level_rates)
            .where(eq(level_rates.ratecard_id, updatedRateCard.id));
  ...
  delete: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await db.transaction(async (tx) => {
        ...
        await tx.delete(level_rates)
          .where(eq(level_rates.ratecard_id, input));

        // Delete rate card
        const [rateCard] = await tx.delete(rate_cards)
          .where(eq(rate_cards.id, input))
          .returning();
