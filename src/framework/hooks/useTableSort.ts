...
export function useTableSort<T extends Record<string, any>>(items: T[]) {
	...
	const sortedItems = React.useMemo(() => {
		...
		return [...items].sort((a, b) => {
			...
			// Handle date objects
			if (a[key as string] instanceof Date && b[key as string] instanceof Date) {
			...
			if (Array.isArray(a[key]) && Array.isArray(b[key])) {
				const aStr = a[key].map((item: any) => item.name || item).join(",");
				const bStr = b[key].map((item: any) => item.name || item).join(",");
